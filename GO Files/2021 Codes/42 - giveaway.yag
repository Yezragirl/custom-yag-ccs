...
{{$desc =print $desc "\n\n**React with " $gEmoji " to enter Giveaway **"}}{{editMessage $chan . (cembed "title" "**Giveaway Started !**" "description"  $desc "color" 1879160 "footer" (sdict "text" (joinStr "" "ID: " $uID " | Giveaway Ends " )) "timestamp" $giveawaySdict.expiresAt)}}
...
{{with getMessage $chan $msg}}{{editMessage $chan $msg (cembed "title" "**Giveaway Cancelled !**" "description" (joinStr "" ">>> **Prize:** "  $prize) "footer" (sdict "text" "Giveaway Cancelled") "color" 16711680 )}}{{end}}Done!
...
{{with getMessage $chan $msg}}{{editMessage $chan $msg (complexMessageEdit "embed" (cembed "title" "**Giveaway Ended !**" "description" $desc "footer" (sdict "text" "Giveaway Ended at ") "timestamp" currentTime "color" 0 ) "content" (print "**Winner(s) :** " $winnerList))}}{{end}}

{{if $countWinners}}{{$message := sendMessageRetID 573860427721605120 (print "Payout:\n**Prize :** " .prize "\n**Winner(s) :** " $winnerList)}}{{addMessageReactions 573860427721605120 $message "✔️" "❌"}}
{{else}}
