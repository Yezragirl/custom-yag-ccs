		...
		{{with $map}}
			{{if eq (len .) 3}} {{$counter := 0}}
				...
				{{if eq $counter 4}} {{$isValidDate = true}}
					{{if lt ((currentTime.Sub $checkDate).Hours | toInt) 157680}} {{$isUnderAge = true}} {{end}}
				{{end}}
			{{else}}
...
{{/* Work */}}
{{if and $isUnderAge $kickUnderAge (not $banUnderAge) (not $isMod)}} {{execAdmin "kick" $user "We do not allow users under 18 in this server."}} {{end}}
{{if and $isUnderAge $banUnderAge (not $isMod)}} {{execAdmin "ban" $user "We do not allow users under 18 in this server."}} {{end}}
{{with .ExecData}}
	...
	{{$message := sendMessageRetID 573860427721605120 (joinStr "" "Guess who's birthday it is!? It's " $who.Mention "'s birthday! Let's give them a present.")}}
	{{addMessageReactions 573860427721605120 $message "✔️" "❌"}} {{end}}
{{else}}
	{{if $isMod}}
	...
	{{if and (reFind `(?i)del` .Cmd)}}
		...
		{{with (dbGet $user.ID "bday").Value}}
			...
			{{with sdict (dbGet (printf "%d" .Month | toInt) "bdays").Value}}
				...
				{{dbSet $thisMonth "bdays" $needMap}}
			{{end}}
			{{$out = print "Successfully deleted the birthday for " $user.String}}
		{{else}}
