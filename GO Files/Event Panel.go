...
{{$db := dbGet .Message.ID "event"}} 
{{$event := sdict}}{{range $k,$v := $db.Value}}{{$event.Set $k $v}}{{end}}

{{$event.number := 2}}
{{$event.name := "test event"}}
{{$event.desc  := "Test desc"}}
{{$color  := red}}
{{$event.style := "PVE"}}
{{$event.team  := 2}}
{{$event.types  := "Maze"}}
{{$event.start := .timehour}}
{{$event.runner  := cslice}}
{{$event.particiants := cslice}}
{{$event.waitlist := cslice}}


{{$embed := cembed 
    "title" (joinStr "-" $event.number $event.name) 
    "description" $event.desc 
    "color" $color 
    "fields" (cslice 
        (sdict "name" "Style" "value" $event.style "inline" true) 
        (sdict "name" "Team Size" "value" $event.team "inline" true) 
        (sdict "name" "Type" "value" $event.types "inline" true) 
        (sdict "name" "Date/Time" "value" $event.start "inline" false) 
        (sdict "name" "Event Runner" "value" (joinStr "" "`" $event.runner "`") "inline" false) 
		(sdict "name" "Particiants" "value" (joinStr "" "`" $event.particiants "`") "inline" false)
		(sdict "name" "Guild ID" "value" (joinStr "" "`" $event.waitlist  "`") "inline" false)) 
	...
	{{$embed := cembed 
		...
		"fields" (cslice 
			...
			(sdict "name" "Particiants" "value" (joinStr "" "`" $eventparticiants "`") "inline" false)
		...
		pass $timeString to CC
		get $timeConverted returned

		{{ $remindertime1 := ($convertedTime.Add (toDuration (mult -3 .TimeHour))).Sub currentTime }}
		{{ $remindertime2 := ($convertedTime.Add (toDuration (mult -3 .TimeHour))).Sub currentTime }}
