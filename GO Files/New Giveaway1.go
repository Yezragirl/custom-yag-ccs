...
{{$desc =print $desc "\n\n**Hosted By :** " $.User.Mention "\n\n**React with " $gEmoji " to enter GiveAway **"}}{{editMessage $chan . (cembed "title" "**Giveaway Started !**" "description"  $desc "color" 1879160 "footer" (sdict "text" (joinStr "" "ID: " $uniqueID " | GiveAway Ends " )) "timestamp" $giveawaySdict.expiresAt)}}
...
{{$desc:=print $desc "\n\n**Hosted By :** " .host}}
{{with getMessage $chan $msg}}{{editMessage $chan $msg (cembed "title" "**Giveaway Ended !**" "description" $desc "footer" (sdict "text" "Giveaway Ended at ") "timestamp" currentTime "color" 0 )}}{{end}}
