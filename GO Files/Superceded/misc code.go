...
-se -color "#BBCCAA" -title "Painting" -desc "If you would like a dino painted, react to this panel with the 🇵 below. There is a limit of 1 painting ticket per day, and 5 dinos per ticket. Please plan ahead." -footer "Yez's Ark Cluster"
...
PREFIX
        {{$getPrefix := exec "prefix"}}
{{$prefix := slice $getPrefix (sub (len $getPrefix) 2) (sub (len $getPrefix) 1)}}
