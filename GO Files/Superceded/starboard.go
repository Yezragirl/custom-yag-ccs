...
{{/* Reaction needed for starboard*/}}
{{$count := 10 }}
{{/* name of your starboard emote*/}}
{{$emoteName := "⭐"}}
{{/* ID of your starboard emote. Leave empty if its a default emote*/}}
{{$emotesID := ""}}
{{/* ID of your starboard channel */}}
{{$starboardID := 616184642302902282 }}
...
{{$match := 0}}
{{/* check all reaction to see if starboard reaction is on it */}}
{{range .ReactionMessage.Reactions}}

{{/* check if emote is starboard emote */}}
{{if eq (toString .Emoji.Name) (toString $emoteName)}}

{{/* if starboard emote meets required count and bot has not reacted */}}
{{if eq .Count $count}}
{{if not .Me}}
{{$match = 1}}
{{end}}{{end}}{{end}}{{end}}
...
{{sendMessage (toString $starboardID) (cembed $RawEmbed) }}
