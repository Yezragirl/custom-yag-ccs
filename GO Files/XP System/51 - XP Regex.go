...
{{if or (le $minimum 0) (le $maximum 0) (le $cooldown 0)}}
	{{sendMessage nil "Please run xpsetup before attempting to use the xp system."}}
{{else}}
{{if not (dbGet .User.ID "pause")}}
	...
	{{if gt $newlevel $currentlevel}}
		{{if eq (toInt $newlevel) 105}}
			{{$embedA := (cembed 
			"title" "You've Ascended!" 
			"color" $color 
			...
			{{sendMessage nil $embedA}}
			{{dbSet .User.ID "XP" 0}}
			{{takeRoleID .User.ID 649424996115939347}}
			{{if reFind `🦕` .Member.Nick}}
				{{editNickname (reReplace `🦕` .Member.Nick `🦖` )}}
				{{giveRoleID .User.ID 737838152512700516}}
			{{else}}
				{{editNickname (joinStr "" .Member.Nick " 🦕" )}}
				{{giveRoleID .User.ID 737838017967816775}}
			{{end}}
		{{else}}
			{{$x := giveRoleName .User.ID (joinStr "" "Level " (toInt $newlevel))}}
			...
			{{if eq .Channel.ID 584957111872782366}}
				{{sleep 1}}
				{{dbSet 18 "counter_count" 1}}
			{{end}}
		{{end}}
