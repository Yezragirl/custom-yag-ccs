{{$args := parseArgs 4 "correct syntax is `setv rule# tier #-of-violations Fine text`"
    ...
    (carg "string" "Text")}}
{{$tiers := cslice 1 2 3}}
{{$rules := cslice 48 56 7 9 14 16 17 18 19 20}}
{{$r := $args.Get 0}}
{{$n := $args.Get 1}}
{{if not (in $tiers $n)}}
...
{{else}}
{{$data := sdict}}
{{$db := dbGet (toInt64 $r) "ruletierinfo"}}
{{if $db}}
{{range $k, $v := $db.Value}}
{{$data.Set $k $v}}
{{end}}
{{end}}
{{$data.Set (joinStr "" "Tier" $n) ($args.Get 2)}}
{{$data.Set (joinStr "" "Tier" $n "Fine") ($args.Get 3)}}
{{$data.Set (joinStr "" "Tier" $n "Warn") ($args.Get 4)}}
{{dbSet (toInt64 $r) "ruletierinfo" $data}}
